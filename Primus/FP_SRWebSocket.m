...
static NSString *const FP_SRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);
static inline void SRFastLog(NSString *format, ...);

@interface NSData (FP_SRWebSocket)
...
@interface NSString (FP_SRWebSocket)
...
@interface NSURL (FP_SRWebSocket)
...
@implementation NSData (FP_SRWebSocket)
...
@implementation NSString (FP_SRWebSocket)
...
NSString *const FP_SRWebSocketErrorDomain = @"FP_SRWebSocketErrorDomain";
...
typedef void (^data_callback)(FP_SRWebSocket *webSocket,  NSData *data);
...
@interface FP_SRWebSocket ()  <NSStreamDelegate>

@property (nonatomic) FP_SRReadyState readyState;
...
@implementation FP_SRWebSocket {
    ...
    __strong FP_SRWebSocket *_selfRetain;
...
- (void)setReadyState:(FP_SRReadyState)aReadyState;
{
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:FP_SRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if (responseCode >= 400) {
        ...
        [self _failWithError:[NSError errorWithDomain:FP_SRWebSocketErrorDomain code:2132 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], SRHTTPResponseErrorKey:@(responseCode)}]];
        return;
    }
    
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:FP_SRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:FP_SRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(FP_SRWebSocket *webSocket,  NSData *data) {
...
- (void)close;
{
    [self closeWithCode:FP_SRStatusCodeNormal reason:nil];
}
...
- (void)_closeWithProtocolError:(NSString *)message;
{
    ...
    [self _performDelegateBlock:^{
        [self closeWithCode:FP_SRStatusCodeProtocolError reason:message];
        dispatch_async(_workQueue, ^{
            [self closeConnection];
        });
    }];
...
- (void)handleCloseWithData:(NSData *)data;
{
    ...
    if (dataSize == 1) {
        ...
        _closeCode = FP_SRStatusNoStatusReceived;
    }
    ...
    switch (opcode) {
        case SROpCodeTextFrame: {
            ...
            if (str == nil && frameData) {
                [self closeWithCode:FP_SRStatusCodeInvalidUTF8 reason:@"Text frames must be valid UTF-8"];
                dispatch_async(_workQueue, ^{
                    [self closeConnection];
                });
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(FP_SRWebSocket *webSocket, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(FP_SRWebSocket *webSocket, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [webSocket _addConsumerWithDataLength:extra_bytes_needed callback:^(FP_SRWebSocket *webSocket, NSData *data) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (bytesWritten == -1) {
            [self _failWithError:[NSError errorWithDomain:FP_SRWebSocketErrorDomain code:2145 userInfo:[NSDictionary dictionaryWithObject:@"Error writing to stream" forKey:NSLocalizedDescriptionKey]]];
             return;
        }
...
- (void)_scheduleCleanup
{
    @synchronized(self) {
        ...
        // This way we'll prevent race conditions between handleEvent and FP_SRWebSocket's dealloc
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!frame) {
        [self closeWithCode:FP_SRStatusCodeMessageTooBig reason:@"Message too big"];
        return;
    }
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
    ...
    if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
        ...
        if (sslCerts) {
            ...
            if (!_pinnedCertFound) {
                dispatch_async(_workQueue, ^{
                    [weakSelf _failWithError:[NSError errorWithDomain:FP_SRWebSocketErrorDomain code:23556 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid server cert"] forKey:NSLocalizedDescriptionKey]]];
                });
                return;
            }
        }
        ...
        case NSStreamEventEndEncountered: {
            ...
            if (aStream.streamError) {
                ...
                if (!_sentClose && !_failed) {
                    ...
                    [self _performDelegateBlock:^{
                        if ([self.delegate respondsToSelector:@selector(webSocket:didCloseWithCode:reason:wasClean:)]) {
                            [self.delegate webSocket:self didCloseWithCode:FP_SRStatusCodeGoingAway reason:@"Stream end encountered" wasClean:NO];
                        }
                    }];
                }
            }
...
@implementation NSURL (FP_SRWebSocket)
...
@implementation NSRunLoop (FP_SRWebSocket)
